...
6. 动态读取任务字段配置，唯一解析计划开始、计划完成和预计工时字段ID。
7. 动态读取任务工作流，唯一解析`待处理`和`已分配`状态ID。
8. 校验日期和正数工时，冻结对象、关系、字段、状态和负责人指纹。
...
1. 创建或复用唯一`【开发】<需求标题>`；创建时使用`parent-id`直接归入来源交付。
...
5. 聚合回读开发任务负责人、日期、工时、状态、两条关系和下一阶段命令。
